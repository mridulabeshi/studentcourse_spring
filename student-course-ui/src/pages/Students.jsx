...
const deptInfo = Object.fromEntries(DEPT_OPTIONS.map((d) => [d.value, d]));

const DEPT_ICONS = {
  WHM: "🌟", ELM: "⚡", DKA: "💀", MHL: "🌿", MAD: "⚔️", TEM: "⚙️",
};

export function DeptBadge({ dept }) {
  const d = deptInfo[dept?.toUpperCase()];
  if (!d) return <span className="badge badge-purple">{dept ?? "—"}</span>;
  return (
    <span className={`badge ${d.color}`}>
...
function Students() {
  ...
  // edit modal
  const [editing, setEditing]       = useState(null); // student obj
  ...
  const addStudent = () => {
    ...
    setSubmitting(true);
    // rollNo is auto-generated by backend: DEPT + (1000+count)
    api.post("/students", { name, email, department })
      .then(() => { setName(""); setEmail(""); setDepartment(""); loadStudents(); toast("Student added ✨"); })
      .catch((e) => toast(e?.response?.data?.message ?? "Failed to add student", "error"))
      .finally(() => setSubmitting(false));
  };

  const openEdit = (s) => {
    setEditing(s); setEditName(s.name); setEditEmail(s.email);
  };
  ...
  // dept counts for summary bar
  const deptCounts = DEPT_OPTIONS.map((d) => ({
    ...d,
    count: students.filter((s) => s.department?.toUpperCase() === d.value).length,
  }));
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 24 }}>
          {deptCounts.map((d) => (
            <button
              ...
              style={{
                ...
                color: deptFilter === d.value ? "var(--accent)" : "var(--text-muted)",
            ...
            >
              ...
              <span style={{
                background: "var(--bg)", borderRadius: "99px", fontSize: "0.7rem",
          ...
          {deptFilter && (
            <button
              ...
              style={{ background: "transparent", border: "1px dashed var(--border-light)", borderRadius: "var(--radius-sm)", color: "var(--text-muted)", cursor: "pointer", fontSize: "0.8rem", padding: "7px 12px", fontFamily: "var(--font-body)" }}
            >✕ Clear filter</button>
          )}
        </div>
      )}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          {department && (
            <span style={{ fontSize: "0.8rem", color: "var(--text-muted)" }}>
              Roll No will be assigned automatically by the backend
            </span>
          )}
        </div>
      </div>
        ...
        <div className="table-wrap">
          {loading ? (
            ...
            <div className="empty-state">
              ...
              <p>{search || deptFilter ? "No students match your filters." : "No students enrolled yet."}</p>
            </div>
          ) : (
            <table className="data-table">
              <thead>
                <tr><th>ID</th><th>Roll No</th><th>Name</th><th>Department</th><th>Email</th><th></th></tr>
              </thead>
              <tbody>
                {filtered.map((s) => (
                  <tr key={s.id}>
                    ...
                    <td>
                      <span style={{ fontFamily: "monospace", fontSize: "0.82rem", color: "var(--text-soft)", letterSpacing: "0.05em" }}>
                        {s.rollNo ?? "—"}
                      </span>
                    </td>
                    ...
                    <td>
                      <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                        ...
                        <span style={{ fontSize: "0.72rem", color: "var(--text-muted)" }}>
                          {deptInfo[s.department?.toUpperCase()]?.label ?? s.department}
                        </span>
                      </div>
                    </td>
            ...
            <div className="flex flex-col gap-16 mb-24">
              ...
              <div className="form-group">
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <span style={{ fontSize: "0.8rem", color: "var(--text-muted)" }}>
                    Department cannot be changed (roll no is tied to it)
                  </span>
                </div>
              </div>
            </div>
...
const modalStyles = {
  ...
  box: {
    ...
    borderRadius: "var(--radius-lg)", maxWidth: 460, padding: 28, width: "90%",
